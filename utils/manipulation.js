export async function detectManipulation(symbol) {
  ...
  try {
    ...
    if (bidSum > askSum * 3) {
      score += 0.5;
      notes.push("Orderbook: aşırı bid yığılması (spoof ihtimali)");
    }
    if (askSum > bidSum * 3) {
      score += 0.5;
      notes.push("Orderbook: aşırı ask yığılması (spoof ihtimali)");
    }
  } catch (e) {
  ...
  try {
    ...
    if (total > 0) {
      ...
      if (ratio > 0.8 || ratio < 0.2) {
        score += 0.5;
        notes.push("Aggressive tek taraflı işlem akışı (layering olasılığı)");
      }
    }
  } catch (e) {
